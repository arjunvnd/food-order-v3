...
async function main() {
  ...
  if (!email && !auth0Sub) {
    console.error(
      '❌  Set SEED_SUPER_ADMIN_EMAIL or SEED_SUPER_ADMIN_AUTH0_SUB before running the seed.',
    );
    process.exit(1);
  }
  ...
  if (existing) {
    console.log(
      `ℹ️   Mall "${mallName}" already exists (id: ${existing.id}) — skipped`,
    );
  } else {
